...
/**
 * Script rápido para crear una API Key (versión ES modules)
 * 
 * Uso: node crear_api_key.mjs [JWT_TOKEN] [NOMBRE_APP] [PERMISOS]
 * 
 * Ejemplo: node crear_api_key.mjs "tu_jwt_token" "Mi App" "clientes:read,clientes:write"
 */
...
async function crearApiKey() {
  ...
  const nombreApp = args[1] || 'Plataforma Externa';
  const permisosInput = args[2] || 'clientes:read,clientes:write,vendedores:read';
  const limite = args[3] ? parseInt(args[3]) : 2000;
  ...
  if (!jwtToken) {
    ...
    console.log('');
    console.log('Uso:');
    console.log('  node crear_api_key.mjs [JWT_TOKEN] [NOMBRE_APP] [PERMISOS] [LIMITE]');
    console.log('');
    console.log('Ejemplo:');
    console.log('  node crear_api_key.mjs "tu_jwt_token_aqui" "Mi Sistema" "clientes:read,clientes:write" 2000');
    console.log('');
